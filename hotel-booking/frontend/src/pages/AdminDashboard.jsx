...
export default function AdminDashboard() {
  ...
  const [expanded, setExpanded] = useState({
    ...
    bookings: path.includes('bookings') || path.includes('new') || path.includes('qr-checkin')
  });
  ...
  const getPageTitle = () => {
    ...
    if (path.includes('qr-checkin')) return 'Quét QR Check-in';
      ...
      <aside style={{ 
        ...
        <nav style={{ flex: 1, padding: '0 16px', overflowY: 'auto' }}>
          ...
          <SidebarGroup 
            ...
            active={path.includes('booking') || path.includes('qr-checkin')}
          >
            ...
            <SidebarSubLink to="/admin/qr-checkin" label="Quét QR Check-in" active={path.includes('qr-checkin')} />
          </SidebarGroup>
        ...
        <header style={{ 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '32px' }}>
             ...
             >
                ...
                <span style={{ position: 'absolute', top: '-2px', right: '-2px', background: '#ef4444', color: '#fff', fontSize: '10px', padding: '2px 5px', borderRadius: '10px', border: '2px solid #fff', fontWeight: '800' }}>5</span>
                
                {/* Luxury Notifications Dropdown */}
                {showNotifications && (
                  <div style={{
                    position: 'absolute', top: '40px', right: '-80px', width: '380px',
                    backgroundColor: '#fff', borderRadius: '18px', boxShadow: '0 15px 40px rgba(15,23,42,0.15)',
                    border: '1px solid #e2e8f0', zIndex: 500, overflow: 'hidden',
                    cursor: 'default'
                  }} onClick={e => e.stopPropagation()}>
                    <div style={{ padding: '18px 24px', borderBottom: '1px solid #f1f5f9', background: '#0a0f1d', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <strong style={{ fontSize: '12px', letterSpacing: '1px' }}>🔔 THÔNG BÁO MỚI</strong>
                      <span style={{ background: 'rgba(255,255,255,0.2)', padding: '3px 8px', borderRadius: '50px', fontSize: '10px', fontWeight: '800' }}>5 MỤC ƯU TIÊN</span>
                    </div>
                    
                    <div style={{ maxHeight: '340px', overflowY: 'auto' }}>
                      <NotificationItem 
                        icon="fa-comment-alt" iconColor="#ef4444"
                        title="Yêu cầu hỗ trợ khẩn cấp"
                        desc="Khách hàng Trịnh Văn Nam đang chờ phản hồi tư vấn cọc..."
                        time="10 phút trước"
                        onClick={() => { navigate('/admin/support'); setShowNotifications(false); }}
                      />
                      <NotificationItem 
                        icon="fa-calendar-plus" iconColor="#0ea5e9"
                        title="Đơn đặt phòng mới #BK-207"
                        desc="Khách hàng Lý Gia Kiệt vừa tạo đơn đặt phòng Deluxe..."
                        time="30 phút trước"
                        onClick={() => { navigate('/admin/bookings'); setShowNotifications(false); }}
                      />
                      <NotificationItem 
                        icon="fa-broom" iconColor="#8b5cf6"
                        title="Yêu cầu dọn phòng"
                        desc="Phòng 204 đã Check-out và cần nhân viên buồng phòng dọn dẹp..."
                        time="1 giờ trước"
                        onClick={() => { navigate('/admin/rooms'); setShowNotifications(false); }}
                      />
                      <NotificationItem 
                        icon="fa-clock" iconColor="#10b981"
                        title="Lịch Check-in hôm nay"
                        desc="Khách hàng Bùi Hoàng Việt dự kiến nhận phòng lúc 14:00..."
                        time="2 giờ trước"
                        onClick={() => { navigate('/admin/bookings'); setShowNotifications(false); }}
                      />
                      <NotificationItem 
                        icon="fa-star" iconColor="#eab308"
                        title="Đánh giá mới 5 sao"
                        desc="Khách hàng Phan Thanh Hà vừa gửi đánh giá cực tốt cho resort..."
                        time="5 giờ trước"
                        onClick={() => { navigate('/admin/reviews'); setShowNotifications(false); }}
                      />
                    </div>
                    
                    <div style={{ padding: '12px', borderTop: '1px solid #f1f5f9', textAlign: 'center', background: '#f8fafc' }}>
                      <span style={{ fontSize: '11px', color: '#64748b', fontWeight: '700', cursor: 'pointer' }} onClick={() => setShowNotifications(false)}>Đóng bảng thông báo</span>
                    </div>
                  </div>
                )}
             </div>
...
function NotificationItem({ icon, iconColor, title, desc, time, onClick }) {
  return (
    <div 
      onClick={onClick}
      style={{ 
        padding: '16px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', gap: '15px', 
        alignItems: 'flex-start', cursor: 'pointer', transition: '0.2s', textAlign: 'left'
      }}
      onMouseEnter={e => e.currentTarget.style.backgroundColor = '#f8fafc'}
      onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}
    >
      <div style={{ 
        width: '38px', height: '38px', borderRadius: '50%', backgroundColor: `${iconColor}15`, 
        color: iconColor, display: 'flex', justifyContent: 'center', alignItems: 'center', flexShrink: 0
      }}>
        <i className={`fas ${icon}`} style={{ fontSize: '14px' }}></i>
      </div>
      <div style={{ flex: 1 }}>
        <h5 style={{ fontSize: '13px', fontWeight: '800', color: '#0f172a', margin: '0 0 4px 0', lineHeight: '1.2' }}>{title}</h5>
        <p style={{ fontSize: '12px', color: '#64748b', margin: '0 0 4px 0', lineHeight: '1.4', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{desc}</p>
