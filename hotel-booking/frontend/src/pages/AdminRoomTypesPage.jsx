...
export default function AdminRoomTypesPage() {
  ...
  const handleOpenModal = (type = null) => {
    if (type) {
      setEditingType(type);
      let photosStr = '';
      if (Array.isArray(type.photo_urls)) {
        photosStr = type.photo_urls.join(',');
      } else if (typeof type.photo_urls === 'string') {
        try {
          const parsed = JSON.parse(type.photo_urls);
          if (Array.isArray(parsed)) {
            photosStr = parsed.join(',');
          } else {
            photosStr = type.photo_urls;
          }
        } catch (e) {
          photosStr = type.photo_urls.replace(/[\[\]"]/g, '').trim();
        }
      }
      setFormData({
        name: type.name,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const urlsArray = formData.photo_urls
        ? formData.photo_urls.split(',').map(url => url.trim()).filter(Boolean)
        : [];
      
      const payload = {
        ...formData,
        photo_urls: urlsArray
      };

      if (editingType) {
        await api(`/admin/room-types/${editingType.id}`, { method: 'PUT', body: payload });
      } else {
        await api('/admin/room-types', { method: 'POST', body: payload });
      }
      setShowModal(false);
      load();
    } catch (error) {
            ...
            <div style={{ padding: '24px' }}>
               ...
               <p style={{ fontSize: '14px', color: 'var(--text-muted)', lineHeight: '1.6', marginBottom: '20px', height: '45px', overflow: 'hidden' }}>{type.description}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', fontSize: '13px', fontWeight: '700', color: 'var(--primary)' }}>
                   ...
                   <div>
                     <i className="fas fa-images" style={{ marginRight: '8px', color: 'var(--gold)' }}></i> 
                     {Array.isArray(type.photo_urls) 
                       ? type.photo_urls.length 
                       : (typeof type.photo_urls === 'string' 
                         ? (type.photo_urls.startsWith('[') ? JSON.parse(type.photo_urls).length : type.photo_urls.split(',').filter(Boolean).length) 
                         : 0)} ảnh
                   </div>
                </div>
            </div>
