          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
             ...
             {loading ? (
                Array.from({ length: 2 }).map((_, idx) => (
                   <div key={idx} className="card-luxury" style={{ display: 'grid', gridTemplateColumns: '250px 1fr 200px', padding: '0', overflow: 'hidden' }}>
                      <div className="skeleton-pulse" style={{ height: '100%', minHeight: '200px' }}></div>
                      <div style={{ padding: '30px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                         <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <div className="skeleton-pulse" style={{ width: '120px', height: '22px' }}></div>
                            <div className="skeleton-pulse" style={{ width: '80px', height: '22px', borderRadius: '50px' }}></div>
                         </div>
                         <div className="skeleton-pulse" style={{ width: '180px', height: '14px' }}></div>
                         <div className="skeleton-pulse" style={{ width: '100%', height: '14px' }}></div>
                         <div className="skeleton-pulse" style={{ width: '60%', height: '14px', marginTop: 'auto' }}></div>
                      </div>
                      <div style={{ padding: '30px', background: '#fafbfc', borderLeft: '1px solid rgba(0,0,0,0.03)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '12px' }}>
                         ...
                         <div className="skeleton-pulse" style={{ width: '100%', height: '40px', borderRadius: '10px', marginTop: '10px' }}></div>
                      </div>
                   </div>
                ))
             ) : bookings.length === 0 ? (
                      ...
                      <div style={{ padding: '30px', borderLeft: '1px solid #f1f5f9', display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: 'center', background: '#fafafa' }}>
                         ...
                         <div style={{ fontSize: '24px', fontWeight: '900', color: 'var(--primary)', marginBottom: '20px' }}>{Number(item.total_amount).toLocaleString()}đ</div>
                          ...
                          {['CONFIRMED', 'CHECKED_IN', 'COMPLETED'].includes(item.status) && (
                             <a 
                               href={`http://localhost:5000${item.voucher_pdf_url || `/uploads/vouchers/Voucher_${item.booking_code || `BK${item.id}`}.pdf`}`}
                               target="_blank" 
                             ...
                             >
                                <i className="fas fa-file-pdf" style={{ marginRight: '5px' }}></i> Tải Voucher PDF
                             </a>
                          )}
